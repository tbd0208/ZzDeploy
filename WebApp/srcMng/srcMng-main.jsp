	...
	<form id="sourceMng" class='flex-c dstrForm' :dstr="distributeLevel">
		<div class='topMenu'>
			<div class='dstrStatusBox'>
				<!-- HOME DSTR MARKS -->
				<div id='prjDstr_home' class="dstrMarkHome"><i v-for="x in ['DEV','STG','OPE','OPE']" class='dstrMark' :dstr='x'></i></div>
				<!-- DSTR STATUS BUTTONS -->
				<label v-for="(x,i) in ['DEV','STG','OPE','ALL']" class='dstrStatus' :dstr='x'>
					<input type="radio" class='rectBox' v-model="distributeLevel" name="distributeLevel" :value="x" :accesskey="'QWER'[i]">{{x}}
				</label>
			</div>
			<div class='projects'>
				<!-- PROJECTS -->
...
<%-- 						<label title="<%=Config.SERVER_BACKUP_PRE_PATH%>"><input type="checkbox">BACKUP_PATH</label> --%>
			...
			<div>
				<span>limitDays : </span>
				<select id='limitDays' name='limitDays' v-model="limitDays"><option value='1'>1<option value='3' selected="selected">3<option value='7'>7<option value='31'>31<option value='365'>365<option value=''>전체</select>
		...
		<div id='tabBox-00' class='flex-c list-body'>
			<div class='tabHeadBox'>
				<div class='tabHead workingSet' v-on:click='clickWsTab'>★ WORKING SET <span class='reload btnTxt' v-on:click=reload>[↻]</span></div>
